...
for i = 1 : 10
    ...
    eig_vec_princ_plot = (reshape((eig_vec_princ(:, i)), [28, 28]));

    figure(i);

    subplot(1, 3, 1);
    imagesc(uint8(mu_plot-lambda_max(i)^0.5*eig_vec_princ_plot));
    axis equal;
    title('$\mu-\sqrt(\lambda1)*v1$', 'Interpreter','latex', FontSize=15);
    
    subplot(1, 3, 2);
    imagesc(uint8(mu_plot));
    axis equal;
    title('\mu', Fontsize=15);
